...
class Coub {
  constructor() {
    ...
    this.tokenFile = path.join(__dirname, "./token/coub-token.json");
  }
  ...
  async readTokens() {
    try {
      ...
      if (error.code === "ENOENT") {
        logger.info("coub-token.json file not found, creating new");
        return {};
      }
      logger.error(`Error reading coub-token.json file: ${error.message}`);
      return {};
    }
  }
  ...
  async readAccountData() {
    try {
      const dataFile = path.join(__dirname, "./../data/coub.txt");
